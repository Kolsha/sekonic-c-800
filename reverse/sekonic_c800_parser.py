...
class SPDScaleMode(Enum):
    ...
    RELATIVE = 0x00  # Relative (0.0-1.0)
    AUTO = 0x01  # Automatic
...
class Measurement:
    ...
    # SPD Scale Mode
    spd_scale_mode: Optional[SPDScaleMode] = None
...
class SekonicC800Parser:
    ...
    def _parse_profile_directory(self):
        ...
        for i in range(self.header.profile_count):
            ...
            profile = ProfileEntry(
                name=name, measurement_count=count, measurement_ids=ids
            )
    ...
    def _parse_single_measurement(
        self, record_data: bytes, record_id: int, profile_name: str
    ) -> Measurement:
        ...
        # Parse the SPD Scale Mode from the fixed offset
        scale_mode_offset = 940
        scale_byte = record_data[scale_mode_offset]
        try:
            measurement.spd_scale_mode = SPDScaleMode(scale_byte)
        except ValueError:
            print(
                f"Warning: Unknown SPDScaleMode byte {scale_byte:#02x} for measurement ID {record_id}"
            )
            measurement.spd_scale_mode = None
    ...
    def _extract_primary_values(self, data: bytes, measurement: Measurement):
        ...
        for i, value in float_values:
            ...
            if (
                2000 <= value <= 5000
                and 8 <= i <= 12
                and "cct" not in measurement.values
            ):
            ...
            # CRI Ra values appear around positions 30-40
                ...
                50 <= value <= 100
                and 30 <= i <= 40
                and "cri_ra" not in measurement.values
            ):
    ...
    def _extract_illuminance(self, float_values: List[tuple], measurement: Measurement):
        ...
        if best_match and best_diff < 5:  # Within reasonable tolerance
            ...
            measurement.values["illuminance_lx"] = max(
                illuminance_candidates, key=lambda x: x[0]
            )[1]
    ...
    def _parse_spectral_blocks(self, data: bytes, measurement: Measurement):
        ...
        if spd_points:
            measurement.spectral_blocks["SPD_COMBINED"] = SpectralDataBlock(
                magic=0x82, index=0, data=spd_points
            )
...
def plot_values(title, values):
    ...
    # Create wavelength array from 380 to 780 nm
    # Assuming the data points are evenly distributed across this range
    wavelengths = np.linspace(380, 780, len(values))
    
    # Create the plot
    plt.figure(figsize=(10, 6))
    plt.plot(wavelengths, values, label=title, linewidth=1.5)
    
    # Set labels and title
    plt.xlabel('Wavelength (nm)')
    plt.ylabel('Spectral Power Density (W·m⁻²·nm⁻¹)')
    plt.title(f'Spectral Power Distribution - {title}')
    
    # Auto-scale axes based on data
    plt.xlim(380, 780)  # Fixed x-axis range as requested
    plt.ylim(0, max(values) * 1.1)  # Auto-scale y-axis with 10% padding
...
def main():
    ...
    try:
        ...
        for i, measurement in enumerate(measurements, 1):
            ...
            print(
                f"  Illuminance: {measurement.illuminance_lx:.1f} lx"
                if measurement.illuminance_lx
                else "  Illuminance: N/A"
            )
            print(
                f"  CRI Ra: {measurement.cri_ra:.1f}"
                if measurement.cri_ra
                else "  CRI Ra: N/A"
            )
            print(
                f"  CIE (x,y): ({measurement.cie_x:.4f}, {measurement.cie_y:.4f})"
                if measurement.cie_x and measurement.cie_y
                else "  CIE: N/A"
            )
